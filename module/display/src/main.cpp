...
constexpr auto display_dev = DEVICE_DT_GET(DT_CHOSEN(zephyr_display));

constexpr auto SCREEN_HEIGHT = 320;
constexpr auto SCREEN_WIDTH = 172;
constexpr auto BYTES_PER_PIXEL = 2;
constexpr auto IMAGE_SIZE = SCREEN_HEIGHT * SCREEN_WIDTH * BYTES_PER_PIXEL;
constexpr auto REFRESH_RATE = 30;
constexpr auto REFRESH_PERIOD = K_MSEC(1000 / REFRESH_RATE);

alignas(4) std::uint8_t image_buffer1[IMAGE_SIZE / 2];
alignas(4) std::uint8_t image_buffer2[IMAGE_SIZE / 2];
...
void timer_thread(struct k_timer *timer) { k_sem_give(&display_sema); }
...
void canvas_init() {
   ...
   lv_canvas_set_buffer(canvas1, &image_buffer1, 320, 172 / 2,
                        LV_IMG_CF_TRUE_COLOR);
   lv_canvas_set_buffer(canvas2, &image_buffer2, 320, 172 / 2,
                        LV_IMG_CF_TRUE_COLOR);
...
void read_thread() {
   ...
   load:
      ...
      LV_LOG_INFO("Current FPS = %d, Elapsed time = %d", fps,
                  k_cyc_to_ms_floor32(frame_time - last_frame_time));
      ...
      if (read_bytes != IMAGE_SIZE / 2) {
         LV_LOG_ERROR("Failed to read entire selection, only read %d bytes",
                      read_bytes);
      }
      ...
      lv_fs_seek(&file, 4 + (IMAGE_SIZE / 2),
                 LV_FS_SEEK_SET); // skip over random junk
...
K_THREAD_DEFINE(reading_thread, 2048, read_thread, NULL, NULL, NULL, 2, 0, 0);

K_THREAD_DEFINE(flushing_thread, 1024, flush_thread, NULL, NULL, NULL, 2, 0, 0);
